...
class EarthRoverMiniCameraConfig(CameraConfig):
    ...
    def __post_init__(self):
        ...
        allowed_cams = {self.FRONT_CAM_MAIN, self.FRONT_CAM_SUB, self.REAR_CAM_MAIN, self.REAR_CAM_SUB}
        if self.index_or_path not in allowed_cams:
            raise ValueError(
                f"index_or_path must be one of four allowed camera URLS: {allowed_cams}"
            )
