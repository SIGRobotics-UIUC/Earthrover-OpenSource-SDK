...
The below instructions are for interfacing with the EarthRover Mini from within the LeRobot library.
## Install LeRobot 🤗

To install LeRobot, follow our [Installation Guide](./installation)

In addition to these instructions, you need to install the XXX SDK:

```bash
pip install -e ".[DO WE NEED ANYTHING HERE?]"
```

## Connect to the EarthRover Mini

### 1. Ensure the EarthRover Mini is charged, then hold the button at the front for about 6 seconds or until the lights stop changing.
...
Run the following command or API example to calibrate the Earthrover:
...
Do the same steps to calibrate the Earthrover, run the following command or API example:
